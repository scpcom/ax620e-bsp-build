    ...
    while :; do
        ...
        case $res in
            ...
            2) size=$((size-d)); direction="↓";;
                # OK
            0) break;;
            *) fatal_ign "gunzip returned $res while checking '$(relpath "$file")'"; return 1 ;;
        esac
        echo -n "  $size"
    done
    if [ "$at_min" -gt 3 ]; then
        ...
        if [ -z "$fini" ]; then
            fatal_ign 'oscillating gunzip result, giving up.'
            return 1
        fi
    fi
...
unlzmaWithTrailer(){
    ...
    while :; do
        ...
        if [ "$d" -eq 1 ] && [ "$res" -eq 1 ]; then
            at_min=1          # we're 1 below the result
        fi
        d=$(((d+1)/2))
        case $res in
                # 1: too small
...
current unpacking process."
    ...
    if [ -n "$ramfs_gz_start" ]; then
        echo "Found lzma compressed ramdisk at ${ramfs_gz_start}."
        compressed=lzma
    else
        ramfs_gz_start=$( findByteSequence "$piggy")
        if [ -n "$ramfs_gz_start" ]; then
            echo "Found gzip compressed ramdisk at ${ramfs_gz_start}."
            compressed=gz
        fi
    fi
    if [ "$compressed" ]; then
        ...
        if [ "$compressed" = "gz" ]; then
            rqd_ign gunzipWithTrailer "$ramfs_gz_part3" "$ramfs_cpio" "$padding3" "$part3"
        else
            rqd unlzmaWithTrailer "$ramfs_gz_part3" "$ramfs_cpio" "$padding3" "$part3"
        fi
        if [ -e "$ramfs_cpio" ]; then
    ...
    if [ ! "$compressed" ]; then
        ...
        echo "Found uncompressed ramdisk from ${initrd_start} to ${initrd_end}."
        compressed=""
