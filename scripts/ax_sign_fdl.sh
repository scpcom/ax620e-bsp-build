...
python3 ${GERNERAL_BIN}/imgsign/fdl_AX620E_sign.py -i ${PACK_OUTPUT_DIR}/${INPUT_BIN} -fw ${PACK_OUTPUT_DIR}/${INPUT_FW} \
	 -pub ${PACK_OUTPUT_DIR}/public.pem \
	 -prv ${PACK_OUTPUT_DIR}/private.pem -cap 0x047AFE -key_bit 2048 -o ${PACK_INSTALL_DIR}/${PACKED_BIN} -packsize $(printf %x ${PACKED_MAX_SIZE}) $EXTRA_PARAM
